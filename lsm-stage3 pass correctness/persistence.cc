...
public:
	void prepare()
	{
		...
		std::cout<<store.vlog1.tail<<std::endl;
	...
	void test()
	{
		...
		for (i = 0; i < TEST_MAX; ++i)
		{
			...
			{
			case 0:
				if(store.get(i)==std::string(i + 1, 't') ){
				     std::cout<<"get successfully "<<i<<std::endl;
				}
				else{
				     std::cout<<"get failed "<<i<<std::endl;
				}
				break;
			case 1:
				if(store.get(i)==std::string(i + 1, 't') ){
			...
			case 3:
				if(store.get(i)==std::string(i + 1, 's') ){
				     ...
				     std::cout<<"get failed "<<i<<" "<<store.get(i)<<std::endl;
				}
				break;
			default:
				assert(0);
